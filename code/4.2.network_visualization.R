## R 4.3.2
# Rscript 4.2.network_visualization.r HSC
...
celltype <- args[1]
load('./activity/acs_sc')
load(sprintf('./networks/%s.TF.network',celltype))
load(sprintf('./networks/%s.SIG.network',celltype))
...
if(celltype %in% c('PreB','ImmatureB','MatureB')) {
    ...
    NetBID2::draw.targetNet(source_label=use_driver,source_z=DAG_result[DAG_result$geneSymbol==use_driver,sprintf('degree_%s',celltype)], 
                        edge_score = edge_score,pdf_file=sprintf('../scMINER/fig/networks/%s_argetNet_out_%s.pdf',celltype,use_driver),label_cex = 0.4,n_layer=1,source_cex = 0.5, alphabetical_order=FALSE)
}
