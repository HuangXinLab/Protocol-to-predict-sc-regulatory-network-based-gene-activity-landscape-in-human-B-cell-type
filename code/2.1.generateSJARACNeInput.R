...
# 1_seurat2eset #######
load('./B_sub.rdata')
B_sub=RenameIdents(B_sub,"Immature B"="ImmatureB","Mature B"="MatureB")
levels(B_sub) <- c('HSC',"CLP",'Pre-pro-B','Pro-B','Pre-B','ImmatureB','MatureB','Plasma')
B_sub$mergeCycling <- Idents(B_sub)
#expression_data
expression_data<- B_sub@assays$RNA@data 
#cell_metadata: 
data<-B_sub@meta.data
data$cellName<-data$mergeCycling
